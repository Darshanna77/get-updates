...
I monitor NSE corporate announcements and actions for you.

*Available Commands:*
📌 /add - Add company to watchlist
❌ /remove - Remove company from watchlist
...
🔍 **/search <company_name>**
Find NSE companies by name or symbol
...
📋 **/list**
View all companies in your watchlist
...
4. Company is added to watchlist
...
async def add(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Select company to add to watchlist:",
...
async def confirm_add(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Confirm adding company to watchlist."""
    ...
    confirm_message = (
        f"Please confirm adding this company to your watchlist:\n\n"
...
async def process_confirm_add(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if success:
        await query.edit_message_text(
            f"✅ Added {selected['symbol']} ({selected['exchange']}) to watchlist!",
            parse_mode="Markdown"
        )
    else:
        await query.edit_message_text(
            f"⚠️  {selected['symbol']} ({selected['exchange']}) is already in your watchlist",
            parse_mode="Markdown"
        )
...
async def list_watchlist(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for idx, company in enumerate(watchlist, 1):
        response += f"{idx}. {company['symbol']} ({company['exchange']}) - {company['name']}\n"

    response += f"\n_Total: {len(watchlist)} companies_"
    await update.message.reply_text(response, parse_mode="Markdown")
...
async def remove(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.reply_text(
        "Select company to remove from watchlist:",
        reply_markup=reply_markup
    )

    # Store watchlist for this user
    user_id = update.message.from_user.id
    user_search_results[user_id] = watchlist

    return REMOVE_SELECTION


async def confirm_remove(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Confirm removing company from watchlist."""
    query = update.callback_query
    user_id = query.from_user.id
    await query.answer()
...
async def check(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if results["total_checked"] == 0:
        await update.message.reply_text(
            "📭 Your watchlist is empty. Use /add to add companies.",
            parse_mode="Markdown"
        )
    else:
        message = (
...
async def status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status_message = (
        f"📊 *Bot Status:*\n\n"
        f"📋 Companies in watchlist: {len(watchlist)}\n"
...
async def poll_announcements(context: ContextTypes.DEFAULT_TYPE) -> Dict:
    ...
    watchlist = db.get_watchlist()
    results["total_checked"] = len(watchlist)

    if not watchlist:
        return results

    logger.info(f"Polling {len(watchlist)} companies...")
